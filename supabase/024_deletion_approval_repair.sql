-- ORL OT Management System: repair deletion approval slot compaction
-- Run once in Supabase SQL Editor after 023.

begin;

-- Repack Main slots in two phases. Clearing every request-side slot link first
-- prevents the unique assigned_slot_id constraint from colliding while rows move.
...
begin
  ...
  where session_id=p_session_id and slot_type='MAIN' and request_id is not null;
  ...
  where session_id=p_session_id and slot_type='MAIN' and status<>'CLOSED';
...
begin
  ...
  if not found or r.deletion_status<>'PENDING' then raise exception 'No pending deletion request.'; end if;
