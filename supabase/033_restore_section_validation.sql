-- Phase 4: reject incomplete portable backups before destructive restore.
-- Production installation confirmed by operator. Preserve migration 026 restore fields, age and workflow.
begin;

create or replace function public.orl_db_import(p_session_token uuid,p_password text,p_backup jsonb)
returns jsonb language plpgsql security definer set search_path=public,extensions as $$
declare
  u public.orl_users%rowtype; item jsonb; mapped_user uuid; section_name text;
...
begin
  ...
  for item in select * from jsonb_array_elements(p_backup->'requests') loop
    ...
    insert into public.orl_requests(id,request_number,patient_ic,age,mrn,patient_name,surgery,diagnosis,doctor,specialist,sub_specialty,phone,remark,status,confirmed_at,reviewed_by,reviewed_at,review_note,requested_year,requested_month,postpone_count,postpone_history,deletion_status,deletion_reason,deletion_requested_by,deletion_requested_at,created_by,created_at,updated_at,assigned_slot_id)
    values(r.id,r.request_number,r.patient_ic,r.age,r.mrn,r.patient_name,r.surgery,r.diagnosis,r.doctor,r.specialist,r.sub_specialty,r.phone,r.remark,r.status,r.confirmed_at,null,r.reviewed_at,r.review_note,r.requested_year,r.requested_month,r.postpone_count,r.postpone_history,r.deletion_status,r.deletion_reason,null,r.deletion_requested_at,mapped_user,r.created_at,r.updated_at,null);
