-- 034: checked Reassign. Operator confirmed guarded production installation.
-- Install together with the matching frontend; old clients fail safely until refreshed.

begin;

create or replace function public.orl_swap_slots_checked(
  p_session_token uuid,
  p_from uuid,
  p_to uuid,
  p_expected_from_request_id uuid,
  p_expected_to_request_id uuid
)
returns void
language plpgsql
security definer
...
begin
  ...
  if p_expected_from_request_id is null then
    raise exception 'Expected source patient is required. Refresh the schedule and select again.';
  end if;
  -- Validate both snapshots after locks, before any writes (NULL means empty).
  if a.request_id is distinct from p_expected_from_request_id
     or b.request_id is distinct from p_expected_to_request_id then
    raise exception 'The selected OT slots have changed. Refresh the schedule and select again.';
  end if;
...
revoke all on function public.orl_swap_slots_checked(uuid, uuid, uuid, uuid, uuid) from public;
grant execute on function public.orl_swap_slots_checked(uuid, uuid, uuid, uuid, uuid) to anon, authenticated;

-- Prevent stale cached clients from bypassing the checked entry point.
...
begin
  ...
  raise exception 'Please refresh the page to use the updated Reassign action.';
end $legacy$;
